...
const articulosSlice = createSlice({
  ...
  reducers: {
    ...
    hidratarArticulosIniciales(state, action) {
      const { items, categoria } = action.payload || {};
      state.items = items || [];
      state.categoriaActual = categoria || 'todas';
      state.estado = 'listo';
      state.error = null;
    },
  },
...
export const { setCategoriaActual, hidratarArticulosIniciales } =
